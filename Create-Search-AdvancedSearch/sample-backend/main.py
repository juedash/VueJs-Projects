...
def search_materials(
    ...
    for material in materials.values():
        ...
        if max_density is not None and material.density > max_density:
            continue
        if include_elements:
            print("1--", all(elem in material.formula for elem in include_elements))
            if not all(elem in material.formula for elem in include_elements):
                continue
        if exclude_elements:
            print("2--", exclude_elements, any(elem in material.formula for elem in exclude_elements))
            if any(elem in material.formula for elem in exclude_elements):
                continue
        print("insert", material)
